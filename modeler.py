...
def boltzmannfit(x_in, y_in, x0=None, plot=False, kwargs=None, bounds=(-np.inf, np.inf)):
    if x0:
        starting = x0
    else:
...
def boltzmann(x, mu, s, g=1, e_rev=0, npower=1, kpower=1, n0=0, rate=1, kwargs=None):
    ...
    try:
        ...
        if curr is not None:
            pqco = np.math.factorial(int(k)) / (np.math.factorial(int(k) - int(n)) * np.math.factorial(int(n)))
...
def pt(t, r, n, k=4, ninf=1):
    ...
    prt = ninf * (pt) ** n
    qrt = (1 - (ninf * pt)) ** (k - n)
    p = (np.math.factorial(k) / (np.math.factorial(k - n) * np.math.factorial(n))) * prt * qrt
...
def nt(t, inf, zero, tau):
    ...
    eq = '{:.3f} - (({:.3f} - {:.3f}) * exp(-t/{:.2f})))'.format(inf, inf, zero, tau * 1e3)
    return n, eq
...
def full_trace(t, params, ext_args, type=1):
    ...
    for i in range(len(volt)):
        ...
        if 't_ic' in ext_args:
            I += (ext_args['rate_ic'] * (v-ext_args['v_hold']) * np.exp(-xx / ext_args['t_ic']))
            I[0] = 0
    ...
    str_eq = '{:.2E}'.format(g) + ' * ' + eq_peak + ' * ' + eq_pqco + ' * (p^{})'.format(n) + ' * (q^{})'.format(k-n)
    str_eq_v = ' * (V - {:.2f})'.format(e_rev*1e3)
    str_eq += str_eq_v
    if 't_ic' in ext_args:
        str_eq += ' + ({:.2E} * (V - {:.2f}) * exp(-t/{:.2f}))'.format(ext_args['rate_ic'], ext_args['v_hold'] * 1e3,
                                                                       ext_args['t_ic'] * 1e3)

    str_eq = str_eq + '\n' + 'p = ' + eq_p + ', q = ' + eq_q + ', q1 = ' + eq_q1 + ', q2 = ' + eq_q2 \
             + '\n' + 'a = ' + eq_a + ', b = ' + eq_b + ', c = ' + eq_c

    #TODO: Change if expensive
    for j in range(len(y)):
        if np.abs(y[j]) < 1e-50: y[j] = 1e-50
        if np.abs(y[j]) > 1e50: y[j] = 1e50
...
def fit_full_trace(x_in, y_in, x0, ext_args, type, bounds=(-np.inf, np.inf)):
    ...
    sse, fitted_curve = func_full_trace(best_fit, ext_args, type, x_in, y_in, full_output=True)
...
def analyze_voltage_clamp(data, args=None):
    ...
    if 'init_params' not in args:
        ...
        print('max , min peaks: ', (max_peaksi, peak_yy[max_peaksi]), (min_peaksi, peak_yy[min_peaksi]))
        print('x0 peak: ', x0_peak)
        estimates_peak, x_fit_peak, y_fit_peak, boltzmann_error = boltzmannfit(volt, peak_yy, x0=x0_peak,
        ...
        if ninit>0:
            ...
            for i,init_param in enumerate(args['init_params']):
                ...
                estimates_peaks[i], x_fit_peaks[i], y_fit_peaks[i], boltzmann_errors[i] = boltzmannfit(volt, peak_yy,
                                                                                                       x0=x0_peaks[i],
                                                                                                       plot=plot,
                                                                                                       kwargs=kwargs,
                                                                                                       bounds=bounds_boltz)
    ...
    ext_args = {'v_half': estimates_peak[0], 'slope': estimates_peak[1],
                'e_rev': estimates_peak[2], 'g': estimates_peak[3]}
    ...
    if 'init_params' not in args:
        ...
        if plot and full_plot is False:
            plt.title('Raw and analysis data for %s' % (data['ion_channel']['channel_name']))
            plt.margins(x=0.1, y=0.1)
        ...
        for i in range(int(n_trace/2)):
            ...
            I2, str_eq_2, best_fit_2, fit_error_2 = fit_full_trace(x2, y2, x0_2, ext_args_2, cur_type, bounds=bounds_2)
            ...
            if plot:
                ...
                title = 'Raw and analized data for %s' % (data['ion_channel']['channel_name'])
                ...
                tau_args = {'v_half': ext_args['v_half'], 'slope': ext_args['slope'], 'v_hold': v_hold}
                plt.figure()
                ...
                plt.plot(v_f*1e3, (1-ninf_f*ninf_f2)**n * I_inf, label='ninf*I_inf')
        ...
        x_scale_f = x_scale
        ext_args_f = {'volt': list(volt), 'v_hold': v_hold, 'trace_size': list(trace_size)}
    ...
    if 'init_params' in args:
        ...
        if ninit > 0:
            ...
            for i, init_param in enumerate(args['init_params']):
                ...
                I_fs[i], str_eq_fs[i], best_fit_fs[i], fit_error_fs[i] = fit_full_trace(xf, yf,
                                                                                        x0_fs[i],
                                                                                        ext_args_f,
                                                                                        cur_type,
                                                                                        bounds=bounds_f)
            ...
            I_f, str_eq_f, best_fit_f, fit_error_f = fit_full_trace(xf, yf, x0_f, ext_args_f, cur_type,
                                                                           bounds=bounds_f)
    else:
        I_f, str_eq_f, best_fit_f, fit_error_f = fit_full_trace(xf, yf, x0_f, ext_args_f, cur_type,
                                                                       bounds=bounds_f)
    ...
    tnow = datetime.now()
    ret_val = {'time':str(tnow) ,'ion_channel': ion_channel, 'graph': graph, 'params': final_args, 'x0': list(x0_f),
               'bounds': list(bounds_f), 'ext_args': ext_args_f, 'error': fit_error_f, 'equation': str_eq_f}
    if fit_error_f['r2']>R2LIMIT:
        ...
        for i in range(len(volt)):
            ...
            if not interp:
                x_fit = np.asarray(xf_interp[trace_size_interp[i]:trace_size_interp[i + 1]])
                y_fit = np.asarray(I_interp_f[trace_size_interp[i]:trace_size_interp[i + 1]])
            else:
        ...
        if plot or full_plot or save_plot:
            ...
            plt.legend(bbox_to_anchor=(1.01, 0.25, 0.2, 0), loc=3, mode="expand", borderaxespad=0., fontsize=8)
            if save_plot:
                plt.savefig(PATH2 + str(graph['id']) + '_' + ion_channel['channel_name'] + '_sample_' + ts +'.png', bbox_inches='tight',
                            format='png')
                plt.close()
            ...
            for i in range(len(v_full)):
                if not interp:
                    y_full = np.asarray(I_interp_f2[trace_size_full[i]:trace_size_full[i + 1]])
                else:
            ...
            plt.margins(x=0.1, y=0.1)
            plt.legend(bbox_to_anchor=(1.01, 0.1, 0.2, 0), loc=3, mode="expand", borderaxespad=0., fontsize=8)
            if save_plot:
                plt.savefig(PATH2 + str(graph['id']) + '_' + ion_channel['channel_name'] + '_it_' + ts +'.png', bbox_inches='tight',
                            format='png')
                plt.close()
            ...
            plt.legend()
            if save_plot:
                plt.savefig(PATH2 + str(graph['id']) + '_' + ion_channel['channel_name'] + '_iv_' + ts +'.png', bbox_inches='tight',
                            format='png')
                plt.close()

            plt.figure()
            alpha_f = alphabeta(v_f, args=[best_fit_f[0], best_fit_f[1]], kwargs={})
            beta_f = alphabeta(v_f, args=[best_fit_f[2], best_fit_f[3], best_fit_f[4]], kwargs={'type':'sigmoid'})
            ...
            plt.legend()
            if save_plot:
                plt.savefig(PATH2 + str(graph['id']) + '_' + ion_channel['channel_name'] + '_ab_' + ts +'.png', bbox_inches='tight',
                            format='png')
                plt.close()
            ...
            plt.legend()
            if save_plot:
                plt.savefig(PATH2 + str(graph['id']) + '_' + ion_channel['channel_name'] + '_tau_' + ts +'.png', bbox_inches='tight',
                            format='png')
                plt.close()
            ...
            plt.plot(v_f * 1e3, (1-ninf_f * ninf_f2)**n * I_inf, label='p_inf*peak_inf')
            ...
            if save_plot:
                plt.savefig(PATH2 + str(graph['id']) + '_' + ion_channel['channel_name'] + '_ninf_' + ts +'.png', bbox_inches='tight',
                            format='png')
                plt.close()
            else:
                plt.show()
